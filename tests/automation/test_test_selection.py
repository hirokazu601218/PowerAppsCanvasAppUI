...
from scripts.testing.validate_selection import APP_ID, ENV_ID, SelectionError, select_record, validate
...
class TestSelectionGate(unittest.TestCase):
    def setUp(self):
        ...
        source.write_text('Screen1: {}\n', encoding='utf-8')
        record = self.repo / self.record
        record.parent.mkdir(parents=True)
        request = self.repo / self.request
        request.parent.mkdir(parents=True)
        request.write_text(json.dumps({
            'schema_version': 1, 'change_id': 'CHANGE-123',
            'target': {'environment_id': ENV_ID, 'app_id': APP_ID},
            'request_text': 'Search the selected department and display one staff member',
            'requirement_ids': ['SCR002-UI-004'],
            'acceptance_criteria': ['Selecting the staff member shows the same number in detail'],
        }), encoding='utf-8')
        self.data = {
            'schema_version': 1, 'change_id': 'CHANGE-123',
            'target': {'environment_id': ENV_ID, 'app_id': APP_ID},
    ...
    def test_valid_selection_runs_only_its_named_cases(self):
        result = validate(self.repo, {self.source, self.record, self.request})
    ...
    def test_postpublish_selects_recorded_cases(self):
        result = select_record(self.repo, self.record)
        self.assertEqual(result['case_ids'], ['IT-SRCH-DETAIL-001', 'UT-SRCH-001'])

    def test_missing_request_fails_closed(self):
        with self.assertRaisesRegex(SelectionError, 'change request must be updated'):
    ...
    def test_changed_test_file_is_not_hidden_by_another_selected_case(self):
        ...
        result = validate(self.repo, {self.source, self.record, self.request, 'e2e/current-app/navigation.test.ts'},
    ...
    def test_unit_test_cannot_be_omitted(self):
        ...
        with self.assertRaisesRegex(SelectionError, 'every changed component needs a unit case'):
            validate(self.repo, {self.source, self.record, self.request})
