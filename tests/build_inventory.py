"""Snapshot v1.08 test record keys and literal labels; not a production schema."""
from pathlib import Path
import json
from preflight import read, index
from fx_check import Engine
root = Path(__file__).resolve().parents[1]
nodes, _ = index(read(root / 'src/staff-master/scrStaffMasterSearch_v1.08.paste.yaml'))
props = {n:c['Properties'] for n,c in nodes.items()}
engine = Engine(props)
groups = {}
for group in ['Staff','Work','Commute','Social','Resident','Tax','Payroll']:
    rows = engine.prop('gal'+group+'108', 'Items')
...
out={'version':'v1.08','scope':'Initial visible test records and literal control labels; production and HTML unknown columns require separate reconciliation','groups':groups,'literal_labels':labels}
