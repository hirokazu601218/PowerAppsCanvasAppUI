...
async function start(page:Page){await page.setViewportSize({width:1366,height:768});await page.goto(process.env.CANVAS_APP_URL!,{waitUntil:'domcontentloaded'});const a=page.frameLocator('iframe[name="fullscreen-app-host"]');await expect(ctl(a,'lblHomePrototype')).toContainText('UI検討用 v1.25',{timeout:60000});return a;}
