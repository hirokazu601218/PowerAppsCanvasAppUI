...
test('AUT-LEDGER-117 six zoom levels, aligned controls and actual two-page A4 PDF',async({page})=>{
  ...
  // v1.21 removed both the old status label and the separate back-to-form action.
  // Wait for the current viewer, then independently verify the generated bytes.
  await expect(ctl('pdfLedger111')).toBeVisible({timeout:90000});
  await expect(ctl('btnLedgerClose111').getByRole('button')).toBeEnabled();
  let pdf='';
