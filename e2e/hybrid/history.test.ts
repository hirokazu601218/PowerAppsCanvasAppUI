...
test('HYBRID-001 histories, zero values, payroll and same-name ledger isolation',async({page})=>{
  ...
  await expect(control('lblLedger_employee_number111')).toHaveText('009900000011');
  await expect(control('lblLedger_monthly_amount_total111')).toHaveText('0');
  await expect(control('lblLedger_route_1_section_from111')).toHaveText('');
  await control('btnLedgerClose111').getByRole('button').click();
  await select('009900000012');
  await expect(control('lblSectionCommute111')).toContainText('0件');
  await expect(control('btnCertificate111').getByRole('button')).toBeDisabled();
  await expect(control('conLedgerModal111')).toBeHidden();
