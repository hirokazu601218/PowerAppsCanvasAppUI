...
test('AUT-LEDGER-117 six zoom levels, aligned controls and actual two-page A4 PDF',async({page})=>{
  ...
  for(const percent of [50,75,100,125,150,200]){
    ...
    await page.screenshot({path:`${process.env.OUTPUT_DIRECTORY}/ledger-${percent}.png`});
  }
  ...
  await page.screenshot({path:`${process.env.OUTPUT_DIRECTORY}/ledger-pdf-preview.png`});
