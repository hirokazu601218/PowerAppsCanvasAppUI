...
KINDS = ('Work', 'Commute', 'Social', 'Tax', 'Payroll')
...
def render(fixtures):
    ...
    return result + 'StaffLedgerTemplate = ' + table(fixtures['LedgerTemplate']) + ';'
...
def validate(fixtures):
    ...
    for row in fixtures['Commute']:
        assert all(m in row for m in ('April','May','June','July','August','September','October','November','December','January','February','March'))
    template = fixtures['LedgerTemplate']
    assert len({r['Field'] for r in template}) == len(template)
    assert not any(r['Field'] in ('employee_name','employee_number','organization') for r in template)
