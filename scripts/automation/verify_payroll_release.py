"""Read-only verification of the Studio-compiled v1.19 release.
...
def verify(path, root=ROOT):
    ...
    manifest = json.loads((root / 'src/staff-master/patches/v1.19/manifest.json').read_text())
    ...
    for change in manifest['changes']:
        ...
        owner = cs[0]
        if prop=='ItemAccessibleLabel' and name in {'galPayroll111','galPayrollModal111','galCommute111'}:
