...
職員マスタ検索は旧編集用App ID `204a48dc-7f23-43dd-b934-4654a3cfa306` の**1アプリ**で実装・隔離データ試験・公開・ユーザー画面確認を行う。表示名は `自動開発_職員マスタ検索`。別の安定版・配布検証版は運用経路から外し、削除候補として保管している。[現在の手順](docs/operations/single-app-workflow.md)／[対象一覧](docs/README.md)を参照。現行アプリの接続はSTUDIO隔離テーブルであり、旧App ID `362ac991-eead-4f07-8373-afdb3ebfdba1` 向けのリンク・config・Actionsを現行アプリ用と扱わない。旧自動化の移行・実証は未完了。下記の旧安定版公開記録とURLは履歴である。
