...
> **現行方針（2026-09-26）**：職員マスタ検索は旧編集用App ID `204a48dc-7f23-43dd-b934-4654a3cfa306` の1件に統一。表示名を `自動開発_職員マスタ検索` に変更。[単一アプリ運用](docs/operations/single-app-workflow.md)を参照。以下にある旧安定版のURL、旧App ID `362ac991-eead-4f07-8373-afdb3ebfdba1` への配布試験は履歴。自動化を現行App IDに移行したことや、本番データへ接続したことを意味しない。
