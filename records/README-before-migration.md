...
職員マスタ検索は旧編集用App ID `204a48dc-7f23-43dd-b934-4654a3cfa306` の**1アプリ**で実装・隔離データ試験・公開・ユーザー画面確認を行う。表示名は `自動開発_職員マスタ検索`。別の安定版・配布検証版は運用経路から外し、削除候補として保管している。[現在の手順](../docs/operations/single-app-workflow.md)／[対象一覧](docs/README-before-migration.md)を参照。現行アプリの接続はSTUDIO隔離テーブルであり、旧App ID `362ac991-eead-4f07-8373-afdb3ebfdba1` 向けのリンク・config・Actionsを現行アプリ用と扱わない。旧自動化の移行・実証は未完了。下記の旧安定版公開記録とURLは履歴である。
...
検索・一覧・固定サマリー・履歴・文字サイズ切替・検索領域の開閉に対応。v1.14では、基本情報の密度と最大6列の可変配置、氏名横の在籍表示、一覧の選択色・区切り線、業務領域を維持した左右余白を反映しました。幅5条件×文字サイズ2条件、追加E2E4件と既存P0の結果は[STATUS](docs/handoff/STATUS-history.md)で確認できます。
...
文書ごとの対象アプリは、[ドキュメント分類一覧](docs/README-before-migration.md)で確認する。

1. [Work運用方針](../docs/operations/work-policy.md)の「1. 方針」
2. [STATUS](docs/handoff/STATUS-history.md)の「現在の作業と読取り対象」
...
- [現行Dataverse・履歴・配置・P0回帰](e2e/hybrid/)、[旧版回復用P0](../other/e2e/staff-master-p0.test.ts)
...
- [貼り付け用v1.11](../other/src/staff-master/scrStaffMasterSearch_v1.11.paste.yaml)
- [画面定義v1.11・管理者用](../other/src/staff-master/scrStaffMasterSearch_v1.11.pa.yaml)
- [PDF保存接続用Power Fx](../other/src/staff-master/CommuteLedger_v1.11_SavePDF_OnTimerEnd.fx)
