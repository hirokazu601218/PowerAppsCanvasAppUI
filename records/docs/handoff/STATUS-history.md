...
- [変更要求](../../../docs/changes/requests/README.md)と[試験選定](../../../docs/testing/change-records/README.md)を同じ変更IDで記録し、[現行アプリの実行手順](../../../docs/operations/current-app-request-to-release.md)で構築・単体／結合・公開・読戻し・要件／基本／詳細／結合ケース仕様の更新を連続させる。[公開後文書照合Actions](../../../.github/workflows/current-app-postpublish-docs.yml)は公開メタデータ・パッケージSHA・文書差分を確認し、選定ケースを公開Playerで再実行する。旧App IDの配布／復旧Actionsは起動せず、現行アプリ向けの自動配布成立とはしない。
- [未決事項一覧 D-07](../../../docs/requirements/open-decisions.md)に総合テスト仕様のシナリオ・時期等の判断待ちを明記。今回はフロー・文書のみ変更し、現行アプリの保存・公開、Dataverse変更、実環境での新ワークフロー実行は行っていない。読取り対象：上記文書、[単一アプリ運用](../../../docs/operations/single-app-workflow.md)、新しいActions・検証スクリプトとテスト。
...
# 現在の作業と読取り対象：単一アプリ公開Playerの実測照合（2026-09-26）

- Microsoft認証後、継続対象App ID `204a48dc-7f23-43dd-b934-4654a3cfa306` の公開Playerを所有者で確認。[観察記録](../verification/current-app-player-20260926.md)と[要件・設計・試験対応表の実測差分](../../../docs/requirements/traceability-matrix.md#公開playerの部分照合と差分2026-09-26)を参照。SCR-002の6タブ、0件時の詳細消去、未保存破棄確認、給与簿の163項目1列／両カレンダー、SCR-005の仮例金額と未登録月の「—」を部分観察した。画面やDataverseの保存・公開は行っていない。
- 不一致：SCR-002からSCR-005へ往復すると検索語は残るが検索結果が1件から初期7件へ戻る。SCR-005の本文をスクロールすると固定すべきサマリーも流れる。SCR-002の表示名は画面要件の文言と異なる。未登録月の金額入り根拠文も再確認する。要件を現行画面へ合わせて変更せず、実装・試験の残件にした。
- SCR-003／004はアプリ内の仮例を観察。SCR-006は一般の確認用権限で入口が無効のため管理者側は未確認。保存版ソースと公開版、5区分の保存先・権限、計算元、専用テスト利用者の検索FAIL／結合BLOCKEDは未解消。今回は文書のみ更新し、現行アプリの自動テスト／Actionsを変更していない。対象文書のリンク・ID・状態表現を確認する。
...
# 現在の作業と読取り対象：改修単位の試験選定と現行アプリE2Eを整備（2026-09-26）

- ユーザー指示：改修部品の単体テストを必須とし、他部品に影響する変更は結合テストを実施。総合テストの業務シナリオ・実施時期は後続で策定する。コード追加・更新は既存自動ケースで不足するときに行う。
- [全体テスト計画](../../../docs/testing/test-policy.md)、[現行プロファイル付き実施要領](../../../docs/testing/test-specification.md)、[試験選定記録](../../../docs/testing/change-records/README.md)、[現行E2E運用](../../../docs/testing/current-app-e2e.md)、[受入計画](../../../docs/acceptance/user-acceptance.md)を対象資料とする。旧ケースIDと旧受入基準のリンクは維持。
- 現行ID `204a48dc-7f23-43dd-b934-4654a3cfa306` を固定した読み取り専用E2Eと差分・選定記録チェックを追加。Actionsの実環境runでの成功、公開版と候補ソースの同一性、branch protectionの必須チェック設定はこの文書追加時点では未確認。旧App IDの結果を現行アプリのPASSとしない。アプリの保存・公開・接続・データは今回変更しない。

---

# 現在の作業と読取り対象：職員マスタ検索を単一アプリに統一（2026-09-26）

- ユーザー指示：編集用アプリだけを継続利用し、配布検証用・別安定版への移送を廃止。Power Apps Makerで編集用App ID `204a48dc-7f23-43dd-b934-4654a3cfa306` の表示名を `自動開発_職員マスタ検索` に変更。3テーブル `_STUDIO` と隔離版HTMLの接続、公開内容、共有・権限は今回変更していない。
- 旧安定版 `362ac991-eead-4f07-8373-afdb3ebfdba1`、配布隔離版 `c5dece33-b799-43be-a55b-3344c83979d9`、初期probe `bd256de5-c7a5-4487-9aef-91ee26d4c946`、2026-09-18残件版はすべて表示名を `削除候補_` で識別。**削除は実施していない**。旧版の正式HTML・元3テーブル・Solutionの10部品は今回変更していない。
- [現行の単一アプリ手順](../../../docs/operations/single-app-workflow.md)。GitHubの旧設定 `config/apps/staff-master.json` とActions/E2Eの旧App ID参照は移行前。旧配布・復旧ワークフローを現行アプリへ転用・実行せず、ID/隔離参照/復旧の再設計と実証を別作業にする。過去の成功runは新App IDに対する合格ではない。元3テーブルのIssue #51は未反映の残件として維持。
- 今回は表示名と文書だけの変更。Power Appsソース改修・再公開・業務受入・単一アプリ向け自動配布の試験は行っていない。文書のリンク・ID・役割の整合のみ確認。過去のSTATUS記録は当時の観測であり、現在の複数アプリ運用方針を示さない。今回の60分中断ルールはユーザーが解除済み。

---

# 現在の作業と読取り対象：安定開発版v1.27復旧確認済み、配布自動化停止（2026-09-24）

- 自動配布run35979556065は旧ソリューション定義の`DatabaseReferences={}`を取込み、公開一覧0件を起こした。ソースとmsappハッシュは一致しており、それだけでは接続参照を検証できない。古い定義はPower Apps release3.26085.11、正常Studio版は3.26092.9。
- Power Apps履歴の正常版58を61へ復元し公開。61がLive。所有者Playerで初期7件、011給与1列163項目・行GUIDを確認。[読戻し35981090795](https://github.com/hirokazu601218/PowerAppsCanvasAppUI/actions/runs/35981090795)は全パッケージSHA256 `4078adc1c7e3c4a7122b2fe8731bb4a3904061f33272ebbb99259bd46e995518`・7ソース一致、`default.cds`の3テーブル参照を確認。
- [専用ユーザーUI run35981343840](https://github.com/hirokazu601218/PowerAppsCanvasAppUI/actions/runs/35981343840)は認証・P0・レスポンシブ画面試験2件PASS。旧回復専用UI run35981127167はテスト前の旧版用静的検査設定で失敗し、上記runで設定を修正して再試験した。
- PR #73候補は配布停止ガードと3テーブル参照の事前検査を追加。古いテンプレートは事前に拒否される。現在のガードを解除しない。隔離コピーで現行メタデータを含む取込みと履歴復元を試し、設計・運用・動的参照を解決してから自動配布経路を再評価する。
- mainへの製品コード昇格・本番利用可判定は未実施。200%・実端末/人の可読性・性能比較・既存警告などの未受入を維持。読取り対象：PR #73、`docs/testing/lightweight-20260924-resume-validation.md`、readback/UI/transaction、無人修正手順。60分ルール解除、元テーブル無変更、Issue #51別残件。

---

# 現在の作業と読取り対象：v1.27 復元版61公開・独立検証中（2026-09-24）

- 改訂配布run 35979556065は取込み/公開/ソース読戻し成功後、専用ユーザーの初期一覧が0件となり失敗。自動復元パッケージでも同じ問題。所有者の最新版でも再現した。
- Power Apps履歴で取込み後の59/60は実行版3.26085.11、正常な58は3.26092.9。古いソリューションメタデータを使った経路は再利用しない。根因は未確定であり、接続/データ参照/実行版を含む完全なメタデータ比較が必要。
- 管理画面で正常なバージョン58（08:57:16 UTC）を61として復元（09:21:56 UTC）、61がLiveを確認。PlayerのRefresh後に7件・004選択、011の給与1列163項目と正しい行GUIDを再確認。ソース一致のみを復旧完了条件にしない。
- `lightweight_transaction.py`は`deployment_enabled`の明示有効化がなければ配布前停止するガードを追加。隔離コピーでメタデータ込みの再検証が成立するまで安定版へ再配布しない。
- 復元版の読戻しrun35981090795と、読取り専用の専用ユーザーUI run35981127167を確認中。成功扱いしない。PR #73はdraft、製品コードのmain昇格なし。
- 読取り対象：PR #73、`docs/testing/lightweight-20260924-resume-validation.md`、lightweight-recovery-ui/readback、無人修正手順。60分ルール解除、元テーブル無変更、Issue #51は別残件を継続。
...
- 2026-09-24 00:36 UTC、専用ユーザー `Power Apps 自動テスト` のMaker画面で隔離 `M_職員基本_STUDIO` に合成行 `009900009995` を作成し、保存済みとレコードIDを確認。職員番号を `009900009994` に更新して保存済みを読戻し、同じレコードを削除。新規のMaker画面で両番号の不在を確認。編集用テーブルの全表示行は職員基本25／通勤6／給与簿7、元テーブルは職員基本25／通勤6／給与簿7で、従来のfixture件数を維持。これはDataverseテーブルのMaker画面CRUDであり、編集コピーアプリのPlayer/Studio内の登録・更新・削除UIを通した試験ではない。専用ユーザーの実効権限については別途代理実行APIの検証runを参照。
...
- 読取り対象: [最新版README](../../../other/src/staff-master/candidates/commute-html-v1.01/README.md)、[設計・項目対応・試験・受入/復旧手順](../../../other/src/staff-master/candidates/commute-html-v1.01/docs/release-1.01.md)、同ディレクトリのソースと要件、`config/dataverse/commute-columns.json`、`tests/fixtures/commute-6.json`。PoCの経緯は`docs/poc/html-report-launch-poc.md`。
...
- 候補ソース／設計／試験／再開位置: [中断記録](../../../other/src/staff-master/candidates/commute-html-v1.00/docs/implementation-status.md)。単体6試験グループPASS、実接続・実権限・A4横2ページPDF・既存機能スモークは未完了。
...
[テスト方針v1.00](../../../docs/testing/test-policy.md)、[テスト仕様書v1.03](../../../docs/testing/test-specification.md)を作成。95ケース定義＋連続スモーク、独立期待値、8表示条件、PDF実体照合、差分回帰を定義した。v1.02ではIssue #23の追加受入、v1.03ではIssue #27の名称維持・工程報告を定義。既存68件のローカル合格は実機合格ではない。
...
Issue #12で発生したテストlocatorの誤りを再発防止ルールへ反映した。表示Labelではなく実操作部品を選ぶこと、`.first()`等で一意性問題を回避しないこと、閉じた領域内の非表示セルを状態保持判定に使わないこと、候補版と復元版の双方でlocatorを検証すること、Playwright証跡でテスト不具合とアプリ不具合を区別することを[テスト仕様書v1.01](../../../docs/testing/test-specification.md#セレクター設計の再発防止ルール)へ追加した。文書だけの変更のためアプリ配布・E2E・P0は対象外。見出しリンク、文書版、変更履歴の整合を確認した。
