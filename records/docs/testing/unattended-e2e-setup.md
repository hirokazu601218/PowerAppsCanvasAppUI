...
> 旧ハンドメイド版の実績。現行単一アプリの新しい実行経路は[現行アプリE2E運用](../../../docs/testing/current-app-e2e.md)。この文書の実行結果を現行App IDの合格と扱わない。
...
`e2e/staff-master-p0.test.ts`は、標準1366×768と縮小900×600で次を確認する。
...
8. 「山田 花子」を選択する
9. 右側詳細に「山田 花子」と職員番号`00990000002`が表示される
10. 画面を900×600へ縮小しても職員基本情報と給与・勤務条件履歴が描画される
11. 基本情報と給与履歴の代表的な隣接項目で文字領域が重ならない
12. 横スクロールが存在し、右端の「勤務時間終了」まで到達できる
13. 検索条件をクリアする
14. 「職員一覧 25件」へ戻る
15. 標準詳細、縮小画面、最終状態のスクリーンショット、動画、トレース、テスト結果を保存する
