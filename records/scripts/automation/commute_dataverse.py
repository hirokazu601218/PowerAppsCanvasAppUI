"""Staged commute setup. Existing OIDC only; no permissions, billing or app edits."""
...
def main():
 ...
 request=json.loads((ROOT/'automation/commute-run.json').read_text())
 ...
 out=ROOT/'artifacts/commute';out.mkdir(parents=True,exist_ok=True)
 ...
 fields=['crb3c_staffnumber','crb3c_fullname','crb3c_hiredate','crb3c_leavedate','crb3c_commuteregistered','crb3c_commutemethod','crb3c_passamount','crb3c_onewayfare',pm['PrimaryIdAttribute']]
 ...
 tables=api("EntityDefinitions?$select=LogicalName,MetadataId&$filter=LogicalName%20eq%20'crb3c_commute'")['value']
 ...
 from commute_schema import table, attributes, relationship, validate_rows, build_fixtures
 entity="EntityDefinitions(LogicalName='crb3c_commute')"
 if not tables:
 ...
 relname='crb3c_staffbasic_commute'
 ...
 assert rel['ReferencedEntity']=='crb3c_staffbasic' and rel['ReferencingEntity']=='crb3c_commute'
 ...
 if request['mode']=='provision':
  api('PublishXml','POST',{'ParameterXml':'<importexportxml><entities><entity>crb3c_commute</entity><entity>crb3c_staffbasic</entity></entities></importexportxml>'})
 result.update(state='TABLE_AND_RELATIONSHIP_VERIFIED',business_columns=84,child_entity_set=meta['EntitySetName'],relationship=relname,delete_behavior='Restrict')
 ...
 result.update(state='SEED_AND_RELATIONSHIP_VERIFIED',seed_count=len(rows),child_counts=counts,parents_unchanged=True,all_84_values_verified=True)
