...
> このファイルはv0.32時点の独立プロトタイプのソース記録です。SCR-005の最新の画面・計算要件は[画面要件定義書8章](../../../docs/requirements/screen-requirements.md#8-scr-005-支給明細画面canvas-ui作成要件)を優先します。2026/09/18追加の俸給支給額・通勤手当・控除内訳と[修正画像](../../../records/docs/design/images/scr-005-payroll-detail-v1.png)は、以下の旧YAMLには未反映です。旧合計式・対象外項目・配置を新UIへそのまま転記しないでください。SCR-003の既存試作とYAML自体は変更していません。既存公開アプリへの統合・実機検証は未実施です。

## 画面要件との対応・UIイメージのデザイン

[画面要件定義書](../../../docs/requirements/screen-requirements.md)の名称に統一します。「数値入りの計算過程」は独立した別画面ではなく、**SCR-005 支給明細画面の表示内容**です。ファイル名はリンク継続のため `attendance-payroll-prototype.md` を維持します。
...
UIイメージは [B案デザイン基準 v1.04](../../../docs/design/design-system.md) を適用します。ヘッダー #073B78、主操作 #0F6CBD、本文 #242424、背景 #F7F9FC、面 #FFFFFF、見出し帯 #EDF2F7、枠 #CBD5E1、角丸4px程度、外余白16px・節間20pxを使用。標準文字はタイトル20px、見出し16px、本文14px、補足12px相当、金額右寄せ。旧DADS資料ではなく現行のB案を基準とします。
