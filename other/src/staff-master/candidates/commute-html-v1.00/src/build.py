...
*{box-sizing:border-box}html,body{margin:0;padding:0;color:#000;background:#fff;font-family:"Yu Gothic","Meiryo","Noto Sans CJK JP",sans-serif;font-size:9pt}
...
h1{text-align:center;font-size:17pt;letter-spacing:3mm;margin:2mm 0 4mm;font-weight:600}.formno{position:absolute;top:5mm;left:2mm;font-size:9pt}
table{border-collapse:collapse;width:100%;table-layout:fixed;border:0.45mm solid #000}td,th{border:0.22mm solid #000;padding:0.65mm 1mm;vertical-align:middle;font-weight:400;overflow-wrap:anywhere;line-height:1.25}th{text-align:center}td{position:relative}.value{display:block;width:100%;font-size:9pt;font-weight:500;white-space:pre-wrap;overflow-wrap:anywhere;line-height:1.2;max-height:100%;min-height:1em}
.inline{display:inline-block;width:auto;min-width:10mm;vertical-align:bottom}.small{font-size:8pt}.num{text-align:right}.center{text-align:center}.vert{writing-mode:vertical-rl;text-align:center;letter-spacing:1mm}.diag{background:linear-gradient(to top right,transparent calc(50% - .15mm),#000 50%,transparent calc(50% + .15mm))}.gap{margin-top:1.2mm}.titlebar{border:.45mm solid #000;border-bottom:0;padding:1mm 2mm;font-size:10pt;height:7mm}
.head{height:18mm}.head td{height:6mm}.routes thead tr{height:8mm}.routes tbody tr{height:13mm}.routes .value{height:11mm}.routes .period .value,.routes .section .value{height:5mm}.routes tfoot tr{height:8mm}.routes .remarks .value{font-size:8pt}.auto td{height:18mm}.combined td{height:10mm}
.shinkansen thead tr{height:7mm}.shinkansen tbody td{height:9mm}.shinkansen tfoot td{height:5mm}.parking thead td{height:8mm}.parking tbody td{height:7mm}.parking tfoot td{height:5mm}.parking-bottom td{height:8mm}.cap td{height:12mm}.monthly th{height:5mm}.monthly td{height:8mm}.monthly .value{height:6mm;font-size:8pt}.decision td{height:9mm}.decision .rules{font-size:8pt;line-height:1.55;padding:1mm 2mm}.refund{border:0}.refund td,.refund th{font-size:8pt;height:8mm}.refund th{height:11mm}.last td{height:13mm}
...
@media print{.toolbar{display:none}#report{margin:0}.page{margin:0;box-shadow:none}body:not(.ready) #report{display:none!important}body:not(.ready) .print-error{display:block;font-size:14pt;padding:15mm}}
'''
...
for i in range(1,9):
 ...
 cells=td('普通交通機関等利用者','rowspan="8" class="vert"') if i==1 else ''
...
park='<thead>'+tr(td('使用する<br>駐車場等','rowspan="2" class="center"')+td('算出の基礎となる駐車場等','colspan="2" class="center"')+td('1箇月当たりの駐車場等の料金に相当する額','rowspan="2" class="center"')+td('1箇月当たりの平均通勤所要回数','rowspan="2" class="center small"')+td('備考（回数券等の場合の駐車場等の料金の算出基礎等）','rowspan="2" class="center"'))+tr(td('駐車場等の利用形態','class="center"')+td('駐車場等の料金','class="center"'))+'</thead><tbody>'
...
park=table([3,10,12,11,6,58],park,'parking')
...
decision=table([2,28,70],tr(td('決定事項','class="vert" rowspan="2"')+td(rules,'rowspan="2" class="rules"')+td(refund,'style="padding:0"'))+tr(td(table([53,20,27],tr(td('1箇月当たりの運賃等相当額及び1箇月当たりの特別料金等相当額が150,000円を超えている場合<br>規則第21条第2項第2号の月数と人事院の定める額（算出基礎）','class="small"')+td('（算出基礎）')+td('　　　　　　　　　円')),'last'),'style="padding:0"')),'decision gap')
...
body='<div class="toolbar"><button id="print" disabled>印刷／PDF保存</button><strong>通勤手当認定簿 HTML版 1.00</strong><p id="status" role="status" aria-live="polite">認定データを読み込んでいます…</p><p>氏名・所属は現在の職員マスタ。未定義欄は空欄です。保存前に対象と内容を確認してください。</p></div><noscript>JavaScriptが無効のため帳票を表示できません。</noscript><p class="print-error">帳票を印刷できません。画面のエラーを確認してください。</p><main id="report" hidden>'+p1+p2+'</main>'
