...
- 画面初回表示と他画面からの再入場、ブラウザーのF5による再起動では検索結果を取得し直し、選択職員が引き継がれ有効なら詳細も取得し直す。職員切替時は当該職員の詳細・履歴・通勤・給与を取得する。保存成功後は保存対象を再取得する。対象年度変更時は対象年度の給与を取得する。タブ切替だけでは再取得せず、画面を開いたままの定期更新は行わない。F5の操作案内を画面に記載しない。
