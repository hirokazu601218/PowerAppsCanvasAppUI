...
- [画面要件1～5章](../requirements/screen-requirements.md#1-画面一覧)の未採番だった確定済み事項へ8つの`COMMON-*`追跡IDを付け、[基本設計の機能割付](../design/basic-design.md#画面要件15章の追跡単位)、[詳細設計12.3／12.4](../design/detailed-design.md)、[対応表](../requirements/traceability-matrix.md)へ対応付けた。新しい業務要件の承認ではない。既存の通勤HTMLの横2ページ／0・NULL／権限、SCR-005試作の最終切捨て規則も詳細化。**要件IDの文書上の設計参照は86/86で、実装可能な業務・物理設計やテストの完了を示さない。**
...
- [要件定義書](../requirements/requirements.md)、[画面要件](../requirements/screen-requirements.md)、[対応表](../requirements/traceability-matrix.md)、[基本設計](../design/basic-design.md)、[詳細設計](../design/detailed-design.md)を同一の単一App IDに合わせて更新。旧816／657コントロールを現行改修直前値から除外し、安定版アプリへの反映という旧完了条件を取り除いた。既存74 IDは維持し、未採番だった既存要件4 IDを追加して計78 IDを対応付けた。
...
- [全体テスト計画](../testing/test-policy.md)、[現行プロファイル付き実施要領](../testing/test-specification.md)、[試験選定記録](../testing/change-records/README.md)、[現行E2E運用](../testing/current-app-e2e.md)、[受入計画](../testing/user-acceptance.md)を対象資料とする。旧ケースIDと旧受入基準のリンクは維持。
...
- [現行の単一アプリ手順](../operations/single-app-workflow.md)。GitHubの旧設定 `config/apps/staff-master.json` とActions/E2Eの旧App ID参照は移行前。旧配布・復旧ワークフローを現行アプリへ転用・実行せず、ID/隔離参照/復旧の再設計と実証を別作業にする。過去の成功runは新App IDに対する合格ではない。元3テーブルのIssue #51は未反映の残件として維持。
