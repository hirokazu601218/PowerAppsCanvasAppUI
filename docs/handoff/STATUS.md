# 現在の作業と読取り対象：1アプリ運用へ切替（2026-09-26）

- ユーザーは編集用アプリ1件で実装・隔離試験・公開・画面確認する方針に変更し、別の配布検証用・安定版アプリへの移送を取りやめた。[現行の単一アプリ運用](../operations/single-app-workflow.md)。旧編集用App ID `204a48dc-7f23-43dd-b934-4654a3cfa306` を表示名 `自動開発_職員マスタ検索` に変更。旧安定版 `362ac991-eead-4f07-8373-afdb3ebfdba1`、2つのprobe、2026-09-18残件版の4アプリは `削除候補_` を付け保管。削除は未実施。
- 以下の旧安定版へ新コピーを作る案、10件Solutionへの自動配布試験は**過去の検討・実施履歴**。旧App IDを指すconfig/workflow/E2Eを現行アプリに向け直したという意味ではない。旧ID向け自動import/publish/restoreは実行しない。単一アプリ向けの自動化は対象ID・接続・復旧方法を再設計・試験するまで未成立。
- 今回の作業は表示名と文書のみ。UI、接続、権限、公開版、データレコードは変更していない。文書だけの変更につきアプリE2Eは対象外、リンクとIDの整合を確認する。職員基本元テーブルのIssue #51を継続。60分中断ルールはユーザー解除済み。

---

# 現在の作業と読取り対象：旧安定版の削除判断（2026-09-25）

- ユーザー方針：編集用アプリを初回コピーした新アプリを今後の安定版にする。旧安定版の削除可否はユーザーが判断。**旧安定版の削除・移行・新コピーの作成はこの読取り調査では実施していない**。
- [比較と削除判断資料](../operations/stable-app-replacement-assessment-20260925.md)：旧安定版Solutionの10ルート部品はCanvas 1／元テーブル3／STUDIOテーブル3／HTML Webリソース3。編集用アプリ本体はその10件に含まれない。両アプリの公開msappは6画面・244実行コントロールだが、編集用は3つのSTUDIOテーブルとStudio隔離HTMLを参照し、旧安定版は元テーブルと正式HTMLを参照する。公開ソース7プロパティと内部生成GUID7件に相違。共有所有者とApp ID/URLも異なる。新コピーはまだ未作成で、本番参照・利用者導線・GitHubの旧App ID参照の移行は未実施。現時点で削除は推奨しない。
- 読取り対象：上記判断資料、[配布試験の直前状況](#現在の作業と読取り対象隔離アプリの停止復旧試験と安定版適用判定2026-09-25)、PR #73。60分中断ルールは今回ユーザー解除済み。
...
- 再実行防止：`.github/workflows/lightweight-maker-copy-revision.yml` はpush起動を削除し、ジョブに `if: false` を設け停止した（commit `0eb586bed9670edc164f573ca55fe7c8559528ff`）。単発の公開フローはOFFのまま。安定版への `deployment_enabled` は未設定（`true` ではない）で、既存スクリプトの拒否条件を維持。Makerの安定版はv61 Live／v62未公開で変化なし。PR #73はdraft、main/成功タグへの昇格なし。
...
- 2026-09-24、同じ`StaffMaster-Automation-Test`環境に、既存の検証用アプリを残したまま`DEPLOYPROBE_AUTOPUBLISH_20260924`（App ID `c5dece33-b799-43be-a55b-3344c83979d9`）をStudioのSave asで追加。Makerでは初期版v1がLive、所有者は山下 浩和、Connections画面には接続なしと表示。管理センターは対象アプリのShare/Details/Deleteのみを提供し、Makerの既存コピーへの追加共有は上限表示で停止。自動化主体への所有権変更・権限追加は実行していない。所有権変更は元所有者をCan Viewにするため別途具体的な承認が必要。新規コピーの自動公開および公開Player変更の受入は未実施。安定版・配布ガードは無変更。
- [検証用アプリの自動公開要求 run 36013918413](https://github.com/hirokazu601218/PowerAppsCanvasAppUI/actions/runs/36013918413)：事前の[読取り専用再照合 run 36013649293](https://github.com/hirokazu601218/PowerAppsCanvasAppUI/actions/runs/36013649293)で3参照・全ソース・244実行定義が承認済み基準と一致。検証用アプリの現行Solutionを再照合した上で自動化サービスプリンシパルから当該アプリのみの公開APIを呼んだがHTTP 403。resultは`status=blocked, touched=false`。Maker版履歴は未公開v7／Live v6のまま、公開Playerはv1.27ホームを表示。安定版には無変更。改訂自動公開は不合格のまま。安定版配布ガードとPR draftを維持。
- 安定版アプリID `362ac991-eead-4f07-8373-afdb3ebfdba1` はv1.27公開中。過去のSolution importでDataverse参照が消えたためStudio履歴から復旧。今回の[読取り専用確認 run 35992833627](https://github.com/hirokazu601218/PowerAppsCanvasAppUI/actions/runs/35992833627)では公開msapp SHA256 `4078adc1c7e3c4a7122b2fe8731bb4a3904061f33272ebbb99259bd46e995518`、M_職員基本/T_基準給与簿/T_通勤の3参照を確認。安定版への自動importは `deployment_enabled` ガードで停止中。
- 別ID `bd256de5-c7a5-4487-9aef-91ee26d4c946` の検証用コピー `DEPLOYPROBE20260924` と専用Solution `LightweightDeploymentProbe` を作成。Solution内のRootComponentはCanvas `cra05_deployprobe20260924_b51e3` の1件だけ。[読取り専用run 35985782401](https://github.com/hirokazu601218/PowerAppsCanvasAppUI/actions/runs/35985782401)で3参照・再パック・元版との全ソース一致を確認。
- [同一版import run 35986462161](https://github.com/hirokazu601218/PowerAppsCanvasAppUI/actions/runs/35986462161)は合格。import後の参照、ソース、244部品の実行定義が一致。メタデータのバイト列はimportにより正規化されるため、意味内容を別に照合。[1プロパティ改訂・復旧run 35992512542](https://github.com/hirokazu601218/PowerAppsCanvasAppUI/actions/runs/35992512542)はラベルTextの改訂をサーバー読戻しで確認し、元版を再importして3参照・全ソース・全実行定義の復旧を確認。元テーブルのデータ行は変更していない。
- 所有者のStudioプレビューと公開Playerで検証用コピーの職員7件・給与163項目/1レコードを確認（2026-09-24 11:37 UTC）。専用ユーザーUI試験と実改訂の公開Player試験は未完了。コピーのShare画面は所有者1名のみで「maximum sharing limit」を表示し追加共有を拒否。自動化サービスプリンシパルはコピーのPower Apps APIで403、PAC canvas downloadも列挙不能。別環境への配布を提示する共有画面を確認した。現在選択可能な別環境は個人Developer（Dataverseあり）と既定環境（Dataverseなし、Maker権限のみ）で、今回のSP公開試験をそのまま移す先はない。安定版の広いSolutionに対するimport試験は未完了。安定版配布ガード、PR #73 draft、main未昇格、成功タグ未作成を維持。200%表示、実端末、業務受入も未完了。
- 読取り対象：[改訂自動化](../operations/lightweight-automation.md)、`scripts/automation/lightweight_isolated_{probe,import,revision}.py`、関連Actions結果、PR #73、下記履歴。今回の60分中断ルールはユーザーが解除済み。
