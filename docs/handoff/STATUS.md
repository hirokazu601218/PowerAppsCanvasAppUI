...
|工程6|StudioでM_職員基本を接続・保存・再読込確認済み。57か所のFx置換は未完了。再読込後に別セッションによる読み取り専用を検出し、強制取得せず停止|
|工程7|未実施。25名への拡充とアプリ回帰は工程6の後|
|再開条件|本人のCanEditは有効。Studioの編集セッション競合の解消が必要。別作業・残存セッションの識別前にオーバーライドしない。接続済み下書きを基準に取得してからFx置換を再開|
