...
- 所有者のStudioプレビューと公開Playerで検証用コピーの職員7件・給与163項目/1レコードを確認（2026-09-24 11:37 UTC）。専用ユーザーUI試験と実改訂の公開Player試験は未完了。コピーのShare画面は所有者1名のみで「maximum sharing limit」を表示し追加共有を拒否。自動化サービスプリンシパルはコピーのPower Apps APIで403、PAC canvas downloadも列挙不能。別環境への配布を提示する共有画面を確認した。安定版の広いSolutionに対するimport試験は未完了。安定版配布ガード、PR #73 draft、main未昇格、成功タグ未作成を維持。200%表示、実端末、業務受入も未完了。
