...
|工程6|Dataverse接続・57か所共通化・旧テストテーブル80定義除去・5件プレビュー検証・下書き保存完了。公開未実施。詳細はdocs/design/dataverse-v1.15-migration.md|
|工程7|run 35080574989で25件投入・値読戻し成功。25件の画面回帰・公開版確認は操作基盤停止のため未実施|
|再開条件|別Work停止を確認して編集ロック解除済み。Studio操作基盤の応答復旧が必要。保存済みv1.15から再開し、旧msappで上書きしない|
