...
> このファイルはv0.32時点の独立プロトタイプのソース記録です。SCR-005の最新の画面・計算要件は[画面要件定義書8章](../requirements/screen-requirements.md#8-scr-005-支給明細画面canvas-ui作成要件)を優先します。2026/09/18追加の俸給支給額・通勤手当・控除内訳と[修正画像](../design/images/scr-005-payroll-detail-v1.png)は、以下の旧YAMLには未反映です。旧合計式・対象外項目・配置を新UIへそのまま転記しないでください。SCR-003の既存試作とYAML自体は変更していません。既存公開アプリへの統合・実機検証は未実施です。
