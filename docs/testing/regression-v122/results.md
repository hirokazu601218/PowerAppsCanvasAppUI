> 2026-09-18追記：PDF残件解消と追加検証の最新結果は [v1.23残件対応](../regression-v123-remaining/results.md) を参照。以下は前回作業の履歴。
...
| [35309682325](https://github.com/hirokazu601218/PowerAppsCanvasAppUI/actions/runs/35309682325) | 公開v1.23 最終候補 | 24 PASS / 1 FAIL（25件） | スモーク3回、給与163項目、画面操作、既存8条件・追加20表示PASS。PDF試験は削除済み状態ラベル待ちで停止 |
...
| 対象 | 条件 | 判定済み実行 |
|---|---|---|
| 職員検索 | 1366×768 / 1920×1080 × 標準/大文字 × 検索欄開/閉（8条件） | 全3回8/8 PASS |
| ホーム、勤務時間報告、期末勤勉支給率、支給明細、メンテナンス | 900×600 / 1100×800 / 1366×768 / 1920×1080（5画面×4幅=20表示） | 第2・第3回4/4パラメーター試験、20/20表示の幾何学検査PASS |
...
v1.23第1候補は04:52:50 UTC、最終候補は05:08:43 UTCのPublish successfulを確認。最終候補は新規ブラウザータブのPlayerでv1.23起動も確認（05:17 UTC）。既存の起動済みタブにはv1.22が残るため、検討時はアプリを開き直す。最終候補はApp checker数式エラー0、7変更プロパティすべてStudio読戻しと一致。
...
最終runは24/25 PASS、1/25 FAIL（96%）。3独立セッションのスモークは3/3 PASS。BUG-SELECTION-001（0件・再選択・初期化）とBUG-DECIMAL-001（2列を含む163項目）は公開後試験で解決確認済み。別の新規Playerでも151,800円表示→管理者初期化→未選択・金額—・再計算無効を05:21 UTCに確認。

残ったAUT-LEDGER-117は6倍率の寸法・操作部品検査を通過後、v1.21で削除された`lblLedgerStatus111`の生成完了表示を90秒待って失敗。PDF生成自体の故障とは未判定で、有効なPDFバイト列・A4横2ページ検査には未到達。次回は現行PDF Viewer/戻る操作の表示を待つ方法に直し、既存のPDFヘッダー限定バイト捕捉とpypdf検査を再実行する。期待文字だけを消してPASSにしない。

95項目の未充足も継続追跡する。リポジトリの`docs/operations/work-policy.md`第1章の1要求60分上限により、この要求の起算を維持し、新しい再実行を追加せず結果を記録する。Issue #54はOpenのまま。終了時点をもって全回帰合格とは扱わない。
