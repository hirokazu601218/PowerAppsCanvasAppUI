...
- 文書版：0.8
- 対象：非常勤給与アプリ
- 定義範囲：画面一覧、画面遷移、操作、引継ぎ情報、権限制御、SCR-002の軽量化要件、SCR-005のUI・試算要件
...
| SCR002-LT-003 | 右領域 | 選択職員サマリーと、基本情報、勤務条件、通勤、社会保険、税固定控除、給与の6タブを表示する |
...
| SCR002-LT-006 | 2列表示 | 基本情報を含む各タブの詳細は、初版では長文項目を含めて2列表示する。長文も黙って省略せず、折返し、行高、Tooltip等で全文確認可能にする。実機試験で欠け・重なり・著しい可読性低下がある項目だけ、根拠を記録して全幅表示へ変更できる |
...
| SCR002-UI-003 | 「ホーム」、支給対象月、文字サイズ切替、支給明細への遷移、「詳細表示」、「変更を破棄」、「保存」など存続する操作をヘッダー直下の操作領域へ集める。操作領域は本文とともにスクロールする。表示条件と権限制御は既存要件に従う |
...
「給与」タブから開く基準給与簿は、SCR-005支給明細とは別の職員台帳機能とする。

| 要件ID | 要件 |
|---|---|
| SCR002-PAY-001 | 基準給与簿の163項目を定義順どおり縦方向へ表示し、重複・欠落を許さない |
| SCR002-PAY-002 | 支給月はデータと表示年度から表示月コレクションを生成し、横方向の列として動的表示する。月ごとの固定コントロールを12組作らない |
| SCR002-PAY-003 | 初版では採用前・退職後を理由とする月列非表示を行わない。表示年度、年度開始月、未登録月、同月複数レコードの扱いは実装時にデータ契約と照合して確定する |
| SCR002-PAY-004 | 0、負数、空欄、未登録を区別する。縦項目×横支給月の末尾へ到達でき、固定見出しとセルの対応を誤認しないこと |
| SCR002-PAY-005 | 163×月数のセルコントロールを静的作成せず、縦ギャラリーと横ギャラリー等の仮想化された反復構造を使用する |

### 9.5 画面領域比率

Power Appsが描画する親領域内に業務画面コンテナーを置く。ブラウザー外枠やOSウィンドウ自体の寸法を制御する要件ではない。
