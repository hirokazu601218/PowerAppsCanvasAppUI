...
| AUT-026 | 成功タグ確定時、検証済み受領記録からルートREADME.mdの成功版・検証run・Issue・公開日時を自動生成し、文書専用ブランチとPRを経てmainへ統合する。全7ゲート合格を必須とし、失敗候補を最新成功版として記録しない。マーカー外の手書き部分を保持する。ActionsのPR作成禁止が明示された場合はPENDING_WORK_PRと生成ブランチ/SHAを記録し、Workが追加確認なしにPR作成・統合を完了する。不正マーカー・未知のPR作成/統合失敗は文書更新未完了として停止する。mainのREADME照合まで依頼全体の完了とは扱わない。 |
