...
| AUT-026 | 成功タグ確定時、検証済み受領記録からルートREADME.mdの成功版・検証run・Issue・公開日時を自動生成し、文書専用ブランチとPRを経てmainへ統合する。全7ゲート合格を必須とし、失敗候補を最新成功版として記録しない。マーカー外の手書き部分を保持し、不正マーカー・PR作成/統合失敗は文書更新未完了として記録する。 |
...
AUT-026はIssue #23の実装対象。最終確定処理でREADME更新PRまで確認し、アプリ合格と文書更新の成否を別に記録する。既存承認はAUT-004に従い有効であり、2026-09-16の開発・テスト・公開・文書更新指示について再確認は不要。
