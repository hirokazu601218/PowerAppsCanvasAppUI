...
10. 確定処理が`scripts/automation/readme_release.py`でREADMEの成功版欄と専用ブランチを生成する。ActionsにPR作成が許可されている場合はPR作成・統合まで行う。現在のリポジトリでは禁止されているため、その既知の拒否だけを`PENDING_WORK_PR`として`artifacts/release/readme-update.json`とIssueに記録する。ブランチ名・生成コミットを残し、成功タグ確定と文書未完了を区別する。未知のエラーは失敗として停止する。手書き本文は保持し、README更新でアプリを再配布しない。
11. `PENDING_WORK_PR`では、このWorkが追加確認なしに生成ブランチの差分がREADMEのみであること、成功タグと版・run・公開日時が一致することを確認し、PR作成・生成コミットSHAを固定した統合を実行する。Actionsの緑表示だけで依頼全体の完了とは扱わない。WorkがmainのREADME、成功タグ、公開版、Actions、文書PRを確認し、STATUSと変更履歴へ結果を反映してこのWorkで報告する。未知のPR作成/統合エラーも原因を確認してから既存の承認範囲で対処する。公開アプリの合格を取り消したり、自動でリポジトリ権限を緩めたりしない。
