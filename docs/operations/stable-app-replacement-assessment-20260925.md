> **2026-09-26 方針変更**：本書の「編集用をコピーして新安定版にする」案は採用しない。現行は[単一アプリ運用](single-app-workflow.md)。以下の差分と旧アプリ削除条件は2026-09-25時点の調査履歴。旧安定版は表示名 `削除候補_自動開発_職員マスタ検索`、旧編集用は表示名 `自動開発_職員マスタ検索` に変更したが、どちらもApp IDとデータ接続はそのままで、アプリ本体は削除していない。
