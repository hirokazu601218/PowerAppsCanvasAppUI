> **廃止済みの運用案（2026-09-26）**。現在は[単一アプリ運用](single-app-workflow.md)を適用する。この文書の3アプリへの配布手順を新規作業に使用しない。以下は2026-09-25時点の経緯として保存する。
...
- [画面要件定義書](../requirements/screen-requirements.md)：今回の画面変更と編集・読み取り区分。
- [Work運用方針](work-policy.md)：作業・承認・報告の共通規則。
- [無人修正・テスト公開手順](staff-master-unattended-runbook.md)：実行ゲートと結果報告。ただし本書の「ユーザーの画面確認→候補固定→配布検証」を当該変更に追加する。
- [最新の作業状況](../handoff/STATUS.md)と[軽量版PR #73](https://github.com/hirokazu601218/PowerAppsCanvasAppUI/pull/73)：配布資格・アプリ構成の実績。履歴記録と今回確定した将来運用を混同しない。
