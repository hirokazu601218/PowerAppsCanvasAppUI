# Power Apps無人修正・テスト・公開基盤 段階別構築計画 v1.00
...
最初の対象は `職員マスタ検索_自動テスト_v1_11` のみとする。既存の無人P0テストを維持しながら、次の順で機能を追加する。
...
### Phase 2：Solution化とGit統合の初期取込み

#### 作業

1. 開発者環境で専用のアンマネージドSolutionを作成する。
2. P0合格済み公開アプリをSolutionへ追加する。
3. 必要な依存部品を列挙し、不要な環境部品を追加しない。
4. SolutionをGitHubリポジトリへPower Platform Git統合する。
5. Git統合が生成したSolution YAML構造を専用の初期取込みブランチへ同期する。
6. GitHub上の既存画面YAMLとの差分を確認し、Git統合側をアプリ全体の正本とする。
7. GitHubからSolutionをパックできることを読み取り・ビルド検査で確認する。

#### ユーザーまたは管理者による初回操作

- Power AppsでSolutionを作成する。
- 対象アプリをSolutionへ追加する。
- GitHub接続を承認する。
- Git統合対象リポジトリ、ブランチ、Solutionを選択する。

これらの操作をブラウザで支援・代行する場合も、接続、認証、権限付与の確定はユーザー承認の範囲内で行う。

#### 完了条件

- `solutions/` 配下にアプリ、画面、Power Fx、Solutionメタデータがそろう。
- GitHub正本からSolutionをパックできる。
- 既存v1.11との差分と移行結果が記録されている。
- P0合格済み公開アプリは変更されていない。
...
| BAS-03 | 同一原因が2回継続 | 自動停止、合格版復元、PR終了、ブランチ保持 |
...
1. GitHub Actionsで既存P0を再実行する。
2. 成功run、App ID、環境ID、公開版を基準記録として確定する。
3. P0合格済み公開アプリを変更せず取得・保全できる方法を確認する。
4. GitHub v1.11との比較項目と差分記録様式を作る。
5. Phase 2で必要になるPower Platform Git統合の利用可否と初回操作を確認する。

Phase 1開始時には、実行前に作業内容と非変更範囲をユーザーへ提示し、承認を得る。
