# Power Apps無人修正・テスト・公開基盤 段階別構築計画 v1.01
...
次の作業は、9ステップ表示のステップ7「小さな修正で修正→公開→テストを一周検証」とする。

1. 業務ロジックへ影響しない表示変更候補と変更専用テストを提示する。
2. ユーザー承認後、active `pa.yaml` を修正する。
3. ステップ6で確立した経路からmsappとSolutionを再構成する。
4. 隔離環境へ反映・公開し、変更専用テストと既存P0を実行する。
5. 両方が合格した場合だけ変更版を確定し、不合格時は直前の合格Solutionへ復元する。

基準・本番アプリ、課金、接続、権限は変更しない。
